...
class OptionSet(object):
    """
    Represents a set of values to sweep over.
    """
    def __init__(self, vals, can_scale=False):
        self.vals = vals
        self.can_scale = can_scale

    def scaled_vals(self, scale_factor):
        """
        If this option `can_scale`, then return scaled vals. Else return values as-is.
        """
        if self.can_scale:
    ...
    def to_array(self, scale_factor=1.0):
        ...
        If can_scale is True, all values will be multiplied by `scale_factor` before being returned.
        """
        return self.scaled_vals(scale_factor)
